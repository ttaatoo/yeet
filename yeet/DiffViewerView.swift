...
private final class DiffWebHostingView: NSHostingView<DiffWebRoot> {
    ...
    override func viewDidMoveToWindow() {
        ...
        // SwiftUI installs this hosting view synchronously during its update
        // pass; `updateAppearance` publishes `usesDarkAppearance`, which must
        // not land inside that pass. The appearance-change variant below
        // arrives from the run loop and needs no hop.
        afterViewUpdate { [model, appearance = effectiveAppearance] in
...
struct DiffViewerView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if isSelected {
                afterViewUpdate { diff.refreshWhenSelected() }
            }
        }

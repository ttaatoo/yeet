...
struct SourceTextEditor: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, STTextViewDelegate {
        ...
        /// Pre-edit UTF-16 range from `willChangeTextIn`. `didChangeTextIn`
        /// gets the same `NSTextRange` after storage has already changed, so
        /// the Swift range must be captured here.
        private var pendingUTF16EditRange: NSRange?
        /// Set by `didChangeTextIn` so `textViewDidChangeText` does not read
        /// `textView.text` (that getter concatenates every paragraph).
        private var appliedIncrementalEdit = false
        ...
        func textView(
            ...
            let utf16Range = pendingUTF16EditRange
            pendingUTF16EditRange = nil
            if let utf16Range,
               let next = FileTextSplice.applying(
               ...
               )
            {
                ...
                appliedIncrementalEdit = true
                return
            }
            copyTextFromView(textView)
            appliedIncrementalEdit = true
        }

        func textViewDidChangeText(_ notification: Notification) {
            pendingUTF16EditRange = nil
            if appliedIncrementalEdit {
                appliedIncrementalEdit = false
                return
            }
        ...
        private func copyTextFromView(_ textView: STTextView) {
...
/// Splice a UTF-16 replacement into `FileTab.text` without reading
/// `STTextView.text`, which walks every paragraph.
enum FileTextSplice {
    static func applying(utf16Range: NSRange, replacement: String, to text: String) -> String? {
        guard utf16Range.location != NSNotFound,
              let range = Range(utf16Range, in: text)
        else { return nil }
        var next = text
    ...
    /// Returns the first mismatch, or nil when every splice case holds.
    static func firstFailingCase() -> String? {
        let cases: [(String, NSRange, String, String?)] = [
            ("hello", NSRange(location: 1, length: 1), "a", "hallo"),
            ("hello", NSRange(location: 5, length: 0), "!", "hello!"),
            ("hello", NSRange(location: 1, length: 3), "", "ho"),
            ("hello", NSRange(location: 0, length: 5), "x", "x"),
            ("", NSRange(location: 0, length: 0), "a", "a"),
            ("a😀b", NSRange(location: 1, length: 2), "x", "axb"),
            ("hello", NSRange(location: 0, length: 99), "x", nil),
            ("hello", NSRange(location: NSNotFound, length: 0), "x", nil),
        ]
        for (text, range, replacement, expected) in cases {
            let got = applying(utf16Range: range, replacement: replacement, to: text)
            if got != expected {
                return "splice \(text) \(range) \(replacement) -> \(String(describing: got)), expected \(String(describing: expected))"
            }
        }
        return nil
    }
...
private final class RestorableScrollView: NSScrollView {
    ...
    /// Temporary: set YEET_SCROLLER_DEBUG=1 to trace scroller geometry.
    private func logScroller(_ tag: String) {
        guard ProcessInfo.processInfo.environment["YEET_SCROLLER_DEBUG"] != nil else { return }

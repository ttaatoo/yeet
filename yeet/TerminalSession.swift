...
final class TerminalSession: NSObject, nonisolated ObservableObject, nonisolated Identifiable {
    ...
    /// Cached per foreground PID so SwiftUI body passes do not `proc_pidinfo`
    /// on every OSC title rewrite.
    var foregroundDirectoryPath: String? {
        guard let foreground = surface.foregroundPid, foreground > 0,
              foreground != shellPid
        else {
            cachedForegroundDirectory = nil
            return nil
        }
        if let cached = cachedForegroundDirectory, cached.pid == foreground {
            return cached.path
        }
        let path = processWorkingDirectory(pid: foreground)
        cachedForegroundDirectory = (foreground, path)
        return path
    }
    ...
    private static func makeLaunchArtifacts(restoredHistory: String?) -> LaunchArtifacts {
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("yeet-terminal-\(UUID().uuidString)", isDirectory: true)
        do {
            ...
            NSLog("yeet: failed to prepare terminal launch files: \(error)")
    ...
    private static func makeLaunchScript(
        ...
        if let pidFileURL {
            ...
            commands.append(
                "(umask 077; printf '%s\\n' \"$$\" > \(POSIXShell.quote(pidFileURL.path)))"
            )
        }
        if let replayFileURL {
            let path = POSIXShell.quote(replayFileURL.path)
        ...
        // app; YEET_TERM names the emulator. Version tracks Yeet, not a
        // vendored Ghostty.
        let version = (Bundle.main.object(forInfoDictionaryKey: "CFBundleShortVersionString") as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let programVersion = (version?.isEmpty == false) ? version! : "0.1.50"
        commands.append("export YEET_TERM=\(POSIXShell.quote("alacritty"))")
        commands.append("export TERM_PROGRAM=\(POSIXShell.quote("Yeet"))")
        commands.append("export TERM_PROGRAM_VERSION=\(POSIXShell.quote(programVersion))")
        if let commandArguments {
            let argv = commandArguments.map(POSIXShell.quote).joined(separator: " ")
            ...
            commands.append("exec \(POSIXShell.quote(shellPath)) -l")
        }
...
extension TerminalSession: TerminalBackendEvents {
    ...
    func terminalDidRequestOpenURL(_ value: String) {
        ...
        case .url(let url):
            // Open the real href from the emulator (OSC 8 can differ from
            // the visible text). Only http(s) leave the terminal as a URL.
            NSWorkspace.shared.open(url)
        }
    }

    /// Classifies a detected terminal link only after proving a local path
    /// exists or the href is http(s). Context menus and Command-click use
    /// this same answer, so neither offers an action it cannot perform.
    func terminalLinkTarget(for value: String) -> TerminalLinkTarget? {
        if let fileURL = existingFileURL(from: value) {
            return .file(fileURL)
        }
        guard let url = URL(string: value),
              BrowserURLPolicy.allowsTerminalWebURL(url)
        else { return nil }
        return .url(url)
    }

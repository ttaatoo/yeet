...
struct ContentView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            ...
            // File-tree and info models stay window-owned next to git so
            // ⇧⌘B hide/show does not reset expansion or the process snapshot.
            if settings.swapSidebars {
                projectSidebar
            } else {
                inspectorSidebar
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            .onScrollGeometryChange(for: StripScrollGeometry.self) { geo in
                ...
                // Geometry callbacks run during the scroll layout pass.
                afterViewUpdate {
                    scrollGeometry = new
...
private struct TabItemChrome: View {
    ...
    var body: some View {
        ...
        .accessibilityLabel(
            isDirty
                ? String(localized: "\(title), unsaved changes")
                : title
        )
    }
}

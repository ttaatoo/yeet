...
final class TerminalMetalRenderer {
    ...
    private static var pipelineCache: [UInt64: CachedPipeline] = [:]
    ...
    init?(device: MTLDevice) {
        guard let queue = device.makeCommandQueue(),
              let pipeline = Self.cachedPipeline(device: device)
        else { return nil }
        self.device = device
        self.queue = queue
    ...
    func render(
        ...
        for _ in 0..<3 {
            ...
            if atlas.reclaimUnused() {
                pendingDirtyRows = nil
                continue
            }
            if atlas.needsLiveCensus, wasPartial {
                pendingDirtyRows = nil
                continue
            }
            break
    ...
    private func kittyTexture(for placement: AlacrittyKittyPlacement) -> MTLTexture? {
        ...
        let width = placement.imageWidth
        let height = placement.imageHeight
        let rowBytes = width * 4
        guard width > 0, height > 0,
              placement.pixels.count == rowBytes * height
        else { return nil }
        ...
        // RGBA is top-down. Metal's origin is bottom-left, matching
        // MTKTextureLoader's previous `.topLeft` option by flipping rows.
        var flipped = Data(count: placement.pixels.count)
        for row in 0..<height {
            let source = row * rowBytes
            let destination = (height - 1 - row) * rowBytes
            flipped.replaceSubrange(
                destination..<(destination + rowBytes),
                with: placement.pixels[source..<(source + rowBytes)]
            )
        }
    ...
    private static func cachedPipeline(device: MTLDevice) -> MTLRenderPipelineState? {
        ...
        guard let compiled = compilePipeline(device: device) else { return nil }
        pipelineCache[key] = compiled
        return compiled.pipeline
    }

    private static func compilePipeline(device: MTLDevice) -> CachedPipeline? {
        ...
        return CachedPipeline(library: library, pipeline: pipeline)
    }

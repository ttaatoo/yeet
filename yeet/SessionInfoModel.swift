...
final class SessionInfoModel: nonisolated ObservableObject {
    ...
    private(set) var projectRootPath = ""
    /// Which rule produced that directory — pinned, the shell's repository,
    /// or the repository the foreground job moved to.
    private(set) var projectRootSource = Project.PanelRootSource.shell
    private(set) var shellName = ""
    private(set) var shellPid: pid_t = 0
    private(set) var processes: [ProcessItem] = []
    private(set) var ports: [PortItem] = []
    ...
    func sync(
        ...
        // Inspector layout calls sync on every pass. ps+lsof is the expensive
        // part; the header button and kill() still go through refresh()
        // unthrottled.
        let snapshotStale =
            lastSnapshotAt.map { Date().timeIntervalSince($0) >= 1 } ?? true
        if pidChanged || snapshotStale {
            refresh()
        }
    }
    ...
    func refresh() {
        ...
        Task { [weak self] in
            let (processes, ports) = await Task.detached(priority: .utility) {
                Self.snapshot(shellPid: pid)
            }.value
    ...
    private nonisolated static func run(_ executable: String, _ args: [String]) -> String {
        ...
        // Dedicated threads drain both pipes. The kernel buffer is ~64KB;
        // leaving stderr unread lets a chatty child block forever on write.
        // Threads (not the cooperative pool) match GitStatusModel.runGit so
        // a snapshot on Task.detached cannot starve its own drain.
        final class PipeBytes: @unchecked Sendable {
            var value = Data()
        }
        let outData = PipeBytes()
        ...
        readers.wait()
        return String(data: outData.value, encoding: .utf8) ?? ""
    }
}

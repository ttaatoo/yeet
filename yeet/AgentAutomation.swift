...
enum KeroAgentKind: String, CaseIterable, Codable, Sendable {
    ...
    /// Arguments that resume the agent's previous native session, appended to
    /// the executable. Agents without a documented resume CLI return nil, and
    /// restore keeps those panes as plain terminals.
    func resumeArguments(sessionID: String) -> [String]? {
        switch self {
        case .claude: return ["--resume", sessionID]
        case .codex: return ["resume", sessionID]
        case .grok: return ["-r", sessionID]
        case .opencode: return ["--session", sessionID]
        case .gemini: return ["--resume", sessionID]
        case .cursor, .aider, .amp, .pi: return nil
        }
    }
...
struct KeroAgentStatus: Equatable, Sendable {
    ...
    let updatedAt: Date
    let processID: pid_t?
    /// Native session identifier from an integration report, so a relaunch
    /// can resume the exact conversation. nil while the agent has not
    /// reported one.
...
final class KeroAgentObservationState {
    ...
    /// Session identifier most recently reported by a native integration.
    /// Kept beside the status so a snapshot saved at quit can resume the
    /// conversation that was live in this pane.
    var integrationSessionID: String?
...
extension TerminalSession {
    ...
    /// Resumes the agent that was live in this pane when the previous launch
    /// quit. The restored shell needs a moment before it accepts input, so
    /// poll for readiness; when the shell never settles the pane stays a
    /// plain terminal — same fallback as an agent with no captured session.
    func scheduleAutomationAgentResume(kind: KeroAgentKind, sessionID: String) {
        guard let arguments = kind.resumeArguments(sessionID: sessionID) else { return }
        Task { @MainActor in
            for _ in 0..<150 {
                if hasExited { return }
                if isShellAvailableForAutomation { break }
                try? await Task.sleep(for: .milliseconds(100))
            }
            guard !hasExited, isShellAvailableForAutomation else { return }
            declareAutomationAgent(alias: kind.rawValue, kind: kind)
            let command = ([kind.executable] + arguments)
                .map(Self.automationShellQuote)
                .joined(separator: " ")
            sendCommand(command + "\r")
        }
    }

    static func automationShellQuote(_ value: String) -> String {
    ...
    func reportAutomationAgent(
        ...
        if let sessionID, !sessionID.isEmpty {
            agentObservation.integrationSessionID = sessionID
        }
        agentObservation.commandGraceDeadline = nil
    ...
    private func clearAutomationAgentStatus() {
        ...
        agentObservation.integrationPhase = nil
        agentObservation.integrationReason = nil
        agentObservation.integrationSessionID = nil
        agentObservation.integrationTurnActive = false
        agentObservation.awaitingInitialPrompt = false
        agentObservation.commandGraceDeadline = nil
    }
    ...
    private func updateAutomationAgentStatus(
        ...
        // Reports from the native integration are the only source of session
        // identifiers; every status emission keeps the latest one.
        let sessionID = agentObservation.integrationSessionID
        if let current = agentStatus,
           current.alias == alias,
           current.kind == kind,
           current.phase == phase,

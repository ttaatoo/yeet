...
final class KeroCLIService {
    ...
    private init() {
        ...
        directoryURL = fileManager.temporaryDirectory
            .appendingPathComponent(
                "yeet-cli-\(ProcessInfo.processInfo.processIdentifier)-\(UUID().uuidString)",
                isDirectory: true
            )
        stateURL = directoryURL.appendingPathComponent("themes.json")
        let socketNonce = UUID().uuidString.prefix(8)
        automationSocketPath = "/tmp/yeet-\(getuid())-\(ProcessInfo.processInfo.processIdentifier)-\(socketNonce).sock"
        ...
        do {
            ...
            NSLog("yeet: failed to prepare CLI state: \(error)")
        }
        ...
        do {
            ...
            NSLog("yeet: failed to start automation socket: \(error)")
        }
    ...
    /// Variables added to every Yeet shell. The app executable's directory is
    /// prepended to the inherited PATH, making `yeet` available without
    ...
    func terminalEnvironment(for sessionID: UUID) -> [String: String] {
        ...
        var environment = [
            "YEET_CLI_STATE": stateURL.path,
            "YEET_CLI_TOKEN": secret,
            "YEET_AUTOMATION": "1",
            "YEET_AUTOMATION_HELP": "yeet +agent explain",
            "YEET_AUTOMATION_SOCKET": automationSocketPath,
            "YEET_AUTOMATION_TOKEN": capability,
            "YEET_TERMINAL_ID": sessionID.uuidString,
        ]
        if let skillURL = try? KeroAutomationSkill.bundledSkillURL() {
            environment["YEET_AUTOMATION_SKILL"] = skillURL.path
        }
    ...
    private func writeState() {
        ...
        do {
            ...
            NSLog("yeet: failed to write CLI theme catalog: \(error)")
        }
    }

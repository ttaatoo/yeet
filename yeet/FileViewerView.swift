...
final class FileTab: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    init(path: String) {
        self.path = path
        // Placeholder until the off-main read finishes. Opening a large file
        // must not stall the tab strip on the main actor.
        content = .unavailable(String(localized: "Opening…"))
        text = ""
        savedText = ""
    ...
    private func applyInitialLoad(_ loaded: LoadedContent, path expectedPath: String) {
    ...
    private nonisolated static func loadedContent(path: String, data: Data?) -> LoadedContent {

...
final class BrowserTab: NSObject, ObservableObject, Identifiable, WKNavigationDelegate, WKUIDelegate {
    ...
    func navigate(to address: String) {
        ...
        guard !trimmed.isEmpty,
              let url = BrowserURLPolicy.destination(for: trimmed),
              BrowserURLPolicy.allowsWebViewNavigation(url)
        else { return }
        errorMessage = nil
    ...
    private func observeWebView() {
        // estimatedProgress ticks many times per second. Do not assign it
        // to @Published: that republishes through Project.objectWillChange
        // and rebuilds ContentView. BrowserView reads WKWebView locally.
        ...
        webView.publisher(for: \.url, options: [.initial, .new])
            .sink { [weak self] url in
                afterViewUpdate {
                    guard let self else { return }
                    let next = Self.displayAddress(for: url)
                    if self.urlString != next { self.urlString = next }
                    self.refreshTitle()
                }
            }
            .store(in: &observations)
        ...
        webView.publisher(for: \.canGoBack, options: [.initial, .new])
            ...
            .sink { [weak self] value in
                afterViewUpdate {
                    guard let self, self.canGoBack != value else { return }
        ...
        webView.publisher(for: \.canGoForward, options: [.initial, .new])
            ...
            .sink { [weak self] value in
                afterViewUpdate {
                    guard let self, self.canGoForward != value else { return }
    ...
    private func refreshTitle() {
        ...
        if let pageTitle = pageTitle?.trimmingCharacters(in: .whitespacesAndNewlines),
           !pageTitle.isEmpty {
            ...
            next = String(localized: "New Tab")
        }
        if title != next { title = next }
    }
    ...
    /// Address-bar / search classification. Kept as a thin wrapper so call
    /// sites stay readable; policy lives in `BrowserURLPolicy` so tests
    /// can exercise it without a WKWebView.
    static func destination(for input: String) -> URL? {
        BrowserURLPolicy.destination(for: input)
    }
    ...
    func webView(
        ...
        if BrowserURLPolicy.allowsWebViewNavigation(url) {
...
struct BrowserView: View {
    ...
    /// Local copy of WKWebView.estimatedProgress. The bar must not read a
    /// @Published BrowserTab field, or each tick rebuilds ContentView.
    ...
    var body: some View {
        ...
        .onAppear {
            afterViewUpdate { address = browser.urlString }
        }
        .task(id: browser.id) {
    ...
    private var progress: some View {
        if browser.isLoading {
                ...
                Rectangle()
                    ...
                    .frame(
                        width: geometry.size.width
                            * max(0.04, min(1, estimatedProgress))
                    )
            }
            .frame(height: 2)
            .transition(.opacity)
            .allowsHitTesting(false)
            .onReceive(
                browser.webView.publisher(
                    for: \.estimatedProgress,
                    options: [.initial, .new]
                )
            ) { estimatedProgress = $0 }
        }

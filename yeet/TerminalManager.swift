...
final class TerminalManager: nonisolated ObservableObject {
    ...
    @Published var selectedProjectID: UUID? {
        willSet {
            ...
            if let selectedProjectID, selectedProjectID != newValue {
                retainDiffs(for: selectedProjectID)
            }
        }
    }
    ...
    /// WKWebView trees and must not stay mounted for every project forever.
    private var retainedDiffProjectIDs: [UUID] = []
    ...
    /// True after the user has confirmed unsaved buffers for this window's
    /// close so `windowShouldClose` can return immediately the second time.
    private var isApprovedWindowClose = false
    /// Forwards SwiftUI's existing window delegate after we intercept close.
    ...
    init() {
        ...
        autosaveObservation = objectWillChange
            ...
            .sink { _ in
                ...
                // A trailing debounce must not encode again after that.
    ...
    private func retainDiffs(for projectID: UUID) {
        retainedDiffProjectIDs.removeAll { $0 == projectID }
        retainedDiffProjectIDs.append(projectID)
        while retainedDiffProjectIDs.count > Self.maxMountedDiffProjects {
            retainedDiffProjectIDs.removeFirst()
        }
    }
    ...
    func attach(to window: NSWindow) {
        self.window = window
        windowCloseGuard.install(on: window, manager: self)
    ...
    func close(_ project: Project) {
        Task { @MainActor [weak self] in
            ...
            guard await project.confirmUnsavedBeforeTeardown(in: self.window) else { return }
    ...
    var hasDirtyBuffers: Bool {
        projects.contains { !$0.dirtyContents.isEmpty }
    }

    func consumeApprovedWindowClose() -> Bool {
        guard isApprovedWindowClose else { return false }
        isApprovedWindowClose = false
        return true
    }

    /// Save / Don't Save / Cancel for every dirty buffer in this window.
    /// Cancel leaves the window and projects untouched.
    func confirmUnsavedBeforeWindowClose() async -> Bool {
        for project in projects {
            if !(await project.confirmUnsavedBeforeTeardown(in: window)) {
                return false
            }
        }
        return true
    ...
    func handleWindowCloseRequest() async {
        guard await confirmUnsavedBeforeWindowClose() else { return }
        isApprovedWindowClose = true
        window?.close()
    }

    /// Confirms dirty buffers in every open window. Used by Quit so Cancel
    /// can abort the whole terminate, not just the key window.
    static func confirmQuit() async -> Bool {
        for manager in registry {
            manager.window?.makeKeyAndOrderFront(nil)
            if !(await manager.confirmUnsavedBeforeWindowClose()) {
                return false
            }
        }
        isQuitting = true
        // Windows are still live. This is the only capture-true save on
        // quit; `willTerminate` must not capture again.
    ...
    private func remove(_ project: Project) {
        ...
        retainedDiffProjectIDs.removeAll { $0 == project.id }
    ...
    func windowClosed() {
        ...
        if Self.registry.contains(where: { $0 !== self }) {
            // Capture while PTYs are still alive, then drop from the registry
            // so SessionStore keeps remaining windows only. Merge this
            // window's scrollback into the sidecar so a non-last close does
            // not wipe it.
            let closing = makeWindowSnapshot(captureTerminalHistory: true)
            Self.registry.removeAll { $0 === self }
            var snapshots: [SessionSnapshot] = []
            var histories = closing.histories
            for manager in Self.registry {
                let window = manager.makeWindowSnapshot(captureTerminalHistory: false)
        ...
        case .pane(let pane):
            ...
            if case .session(let session) = pane.content,
               let history = session.serializedHistory(
                ...
                let key = session.historyKey ?? UUID().uuidString
    ...
    private static func contentSnapshot(
        ...
        case .session(let session):
            // Only a live agent with a reported native session identifier is
            // worth resuming; one without stays a plain restored shell.
            let status = session.agentStatus
            return .session(
                workingDirectory: session.currentDirectoryPath,
                agentKind: status?.kind.rawValue,

...
final class AppConnection {
    ...
    init(environment: [String: String] = ProcessInfo.processInfo.environment) throws {
        guard let statePath = environment["YEET_CLI_STATE"],
              let token = environment["YEET_CLI_TOKEN"],
              !statePath.isEmpty,
              !token.isEmpty
        else {
            throw CLIError.message(
                String(localized: "`yeet` must be run inside a Yeet terminal.")
            )
        }
        stateURL = URL(fileURLWithPath: statePath)
        self.token = token
        automationSocketPath = environment["YEET_AUTOMATION_SOCKET"]
        automationToken = environment["YEET_AUTOMATION_TOKEN"]
        terminalID = environment["YEET_TERMINAL_ID"]
    }
    ...
    fileprivate func loadState() throws -> ThemeState {
        ...
        do {
            ...
            guard state.version == 1 else {
                throw CLIError.message(
                    String(localized: "This yeet CLI does not understand theme catalog version \(state.version).")
                )
            }
            return state
...
private final class ThemeBrowser {
    ...
    func run() throws -> (theme: String, appearance: Appearance)? {
        guard isatty(STDIN_FILENO) == 1, isatty(STDOUT_FILENO) == 1 else {
            throw CLIError.message(
                String(localized: "`yeet +themes` needs an interactive terminal. Use `yeet +themes --list` to print theme names.")
            )
        }
...
private func printHelp() {
    print(
        ...
        Usage:
          yeet
          yeet <command> [arguments...]
          yeet +themes [--dark | --light]
          yeet +themes --list [--dark | --light]
          yeet +pane <command> [options]
          yeet +agent <command> [options]
          yeet +help

        With no arguments, yeet creates a project with a normal login shell.
...
private func run() throws {
    ...
    if arguments.first != "+themes" {
        if let command = arguments.first, command.hasPrefix("+") {
            throw CLIError.message(
                String(localized: "Unknown Yeet command “\(command)”. Run `yeet +help`.")
            )
        }
    ...
    for argument in arguments.dropFirst() {
        ...
        default:
            throw CLIError.message(
                String(localized: "Unknown option “\(argument)”. Run `yeet +themes --help`.")
            )
        }
    }
...
enum KeroCommandLine {
    /// The per-process bridge variables identify invocations from a Yeet
    /// terminal. A `+` action is also CLI-shaped so `yeet +help` works when the
    ...
    static var shouldRun: Bool {
        let environment = ProcessInfo.processInfo.environment
        let hasBridge = environment["YEET_CLI_STATE"]?.isEmpty == false
            && environment["YEET_CLI_TOKEN"]?.isEmpty == false

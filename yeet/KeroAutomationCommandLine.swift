...
enum KeroAutomationCommandLine {
    ...
    private static func runPane(
        ...
        default:
            throw CLIError.message(
                "Unknown +pane command \(command). Run `yeet +pane --help`."
            )
        }
    }
    ...
    private static func runInPane(
        ...
        guard index < arguments.count, arguments[index] == "--" else {
            throw CLIError.message("`yeet +pane run` requires `-- command [arguments...]`.")
        }
    ...
    private static func sendToPane(
        ...
        guard let text else {
            throw CLIError.message("`yeet +pane send` requires --text.")
        }
        var params = targetParams(paneID: paneID)
    ...
    private static func waitForOutput(
        ...
        guard let needle, !needle.isEmpty else {
            throw CLIError.message("`yeet +pane wait-output` requires --contains.")
        }
    ...
    private static func runAgentIntegration(_ arguments: [String]) {
        guard arguments.count == 2 || arguments.count == 3,
              let kind = KeroAgentKind(rawValue: arguments[0]),
              kind == .grok || kind == .claude || kind == .codex,
        ...
        var params: [String: KeroJSONValue] = [
            ...
            "reason": .string("\(kind.displayName) lifecycle hook"),
        ]
        if let sessionID = payload.flatMap(integrationSessionID(in:)) {
    ...
    /// Hooks pipe one JSON object; a human running the command in a terminal
    /// has a TTY on stdin and must not block on an EOF that never comes.
    private static func readIntegrationPayload() -> KeroJSONValue? {
        guard isatty(0) == 0 else { return nil }
        let data = FileHandle.standardInput.readDataToEndOfFile()
        guard !data.isEmpty, data.count <= 1_048_576,
              let event = try? JSONDecoder().decode(KeroJSONValue.self, from: data)
        else { return nil }
        return event
    }
    ...
    private static func runAgentSkill(_ arguments: [String]) throws {
        ...
        default:
            throw CLIError.message(
                "Unknown +agent skill command \(command). Run `yeet +agent skill --help`."
            )
        }
    }
    ...
    private static func printSkillInstall(
        ...
        if changed {
            print("Installed Yeet automation skill.")
        } else {
            print("Yeet automation skill is already installed and current.")
        }
    ...
    private static func printSkillUninstall(
        ...
        if changed {
            print("Uninstalled Yeet automation skill.")
        } else {
            print("Yeet automation skill is not installed.")
        }
        for result in results {
    ...
    private static func skillStateLabel(
        ...
        case .unmanaged: "Present but not managed by Yeet"
    ...
    private static func runAgent(
        ...
        default:
            throw CLIError.message(
                "Unknown +agent command \(command). Run `yeet +agent --help`."
            )
        }
    }
    ...
    private static func startAgent(
        ...
        guard let alias = arguments.first, !alias.hasPrefix("--") else {
            throw CLIError.message("`yeet +agent start` requires an alias.")
        }
        var paneID: String?
        var kind: String?
        var focus = false
    ...
    private static func readAgent(
        ...
        guard let paneID = agent.objectValue?["pane_id"]?.stringValue else {
            throw CLIError.message("Yeet returned an agent without a pane ID.")
        }
    ...
    private static func unknownOption(_ value: String, command: String) -> CLIError {
        .message("Unknown option \(value) for `yeet \(command)`. Run `yeet \(command.components(separatedBy: " ").first ?? command) --help`.")
    }
    ...
    private static func printJSON(_ value: KeroJSONValue) throws {
        ...
        guard let output = String(data: data, encoding: .utf8) else {
            throw CLIError.message("Could not encode Yeet's automation response.")
        }
        print(output)
    }
    ...
    private static func printPaneHelp() {
        ...
        Usage:
          yeet +pane current
          yeet +pane list
          yeet +pane get [--pane ID | --current]
          yeet +pane split [--pane ID] [--left|--right|--up|--down] [--cwd PATH] [--focus]
          yeet +pane run [--pane ID] -- command [arguments...]
          yeet +pane send [--pane ID] --text TEXT [--enter]
          yeet +pane read [--pane ID] [--lines N] [--columns N]
          yeet +pane wait-output [--pane ID] --contains TEXT [--timeout MS]
          yeet +pane protocol
    ...
    private static func printAgentHelp() {
        ...
        Usage:
          ...
          yeet +agent skill <path|print|status|install|uninstall> [options]
          yeet +agent explain
        ...
        explicitly links Yeet's bundled coordination skill into supported
    ...
    private static func printAgentSkillHelp() {
        print("""
        Usage:
          yeet +agent skill path
          yeet +agent skill print
          yeet +agent skill status [--provider PROVIDER] [--json]
          yeet +agent skill install [--provider PROVIDER] [--force] [--json]
          yeet +agent skill uninstall [--provider PROVIDER] [--force] [--json]
        ...
        without --force. Installation uses symlinks to Yeet's app bundle, so
        app updates update the skill without reinstalling it. Reload skills or
        restart an already-running agent after first installation or a Yeet
        update.

        path and print expose Yeet's read-only bundled skill without installing
    ...
    private static func printAgentContract() {
        print("""
        Yeet agent automation contract
        ...
        6. A recognized process Yeet launched is created until its first prompt.
           The model is never asked to report lifecycle, and Yeet never guesses
        ...
        7. The optional yeet-automation Agent Skill teaches this workflow to
           compatible agents. Enable AI in Settings or install it explicitly
           with `yeet +agent skill install`. The AI setting also manages only
           the provider integrations Yeet can use as lifecycle authorities.
           Both the skill and those integrations link to Yeet's app bundle so

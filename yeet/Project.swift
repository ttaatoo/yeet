...
final class Project: nonisolated ObservableObject, nonisolated Identifiable {
    ...
    func panelRoot(
        ...
        if let pinned = customDirectory, TimedFileIO.fileExists(atPath: pinned) {
            return (pinned, .pinned)
        }
        let shellRoot = GitRepositoryLocator.closestGitRepository(containing: cwd) ?? cwd
        ...
        if let foregroundCwd,
           let foregroundRoot = GitRepositoryLocator.closestGitRepository(containing: foregroundCwd),
           foregroundRoot != shellRoot {
            return (
                foregroundRoot,
                .foreground(isWorktree: GitRepositoryLocator.isLinkedWorktree(foregroundRoot))
            )
        }
        return (shellRoot, .shell)
    }

    /// Edited file and diff panes that would lose in-memory changes if this
    /// project were torn down without a save prompt.
    var dirtyContents: [PaneContent] {
        tabs.flatMap(\.allContents).filter(\.isDirty)
    }
    ...
    private func makeSession(
        ...
        // Only chrome-relevant session fields. Terminal I/O and cell-size
        // ticks must not invalidate the project sidebar and tab strip.
        // Coding-agent CLIs rewrite the OSC title many times a second;
        // debounce so those updates do not rebuild ContentView every frame.
        sessionObservations[session.id] = Publishers.Merge(
            session.$title
                .removeDuplicates()
                .debounce(for: .milliseconds(150), scheduler: DispatchQueue.main)
                .map { _ in () },
            Publishers.Merge3(
                session.$workingDirectory.removeDuplicates().map { _ in () },
                // OSC 133 assigns commandLifecycle on prompt, input,
                // executing, and finished. ContentView and RightSidebarView
                // refresh git/file-tree only when completionSequence changes.
                session.$commandLifecycle.map(\.completionSequence)
                    .removeDuplicates()
                    .map { _ in () },
                session.$agentStatus.removeDuplicates().map { _ in () }
            )
        )
        .sink { [weak self] _ in
    ...
    private func confirmCloseUnsaved(_ content: PaneContent, in window: NSWindow?) async -> Bool {
        switch await WorkspaceAlert.unsavedChanges(for: content.title, on: window) {
        case .save:
            content.save()
    ...
    /// Same Save / Don't Save / Cancel sheets as pane close, but the panes
    /// stay mounted: the caller is about to tear the whole project or window
    /// down. Cancel (or a failed save) aborts that teardown so in-memory
    /// edits are not silently dropped.
    func confirmUnsavedBeforeTeardown(in window: NSWindow?) async -> Bool {
        for content in dirtyContents where content.isDirty {
            switch await WorkspaceAlert.unsavedChanges(for: content.title, on: window) {
            case .save:
                content.save()
                guard content.saveError == nil else { return false }
            case .discard:
                continue
            case .cancel:
                return false
    ...
    private func makeContent(
        ...
        case .session(let workingDirectory, let agentKind, let agentSessionID):
            ...
            if let kindName = agentKind,
               ...
               AppSettings.shared.autoResumeAgents {
                session.scheduleAutomationAgentResume(kind: kind, sessionID: sessionID)
            }

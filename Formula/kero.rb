...
class Kero < Formula
  ...
  def install
    ...
    prefix.install "dist/Kero.app"
    # The app executable is itself the bundled `kero` CLI
    # (Kero.app/Contents/MacOS/kero; prepended to PATH inside Kero
  ...
  def caveats
    <<~EOS
      ...
      It is not the notarized app from `brew install egoist/tap/kero`.
        ...
        xattr -dr com.apple.quarantine #{prefix}/Kero.app

      Optional symlink:

        ln -sf #{prefix}/Kero.app /Applications/Kero.app
      ...
      The bundled CLI is #{prefix}/Kero.app/Contents/MacOS/kero
      (already on PATH inside Kero terminals).
  ...
  test do
    assert_path_exists prefix/"Kero.app"
  end
end
